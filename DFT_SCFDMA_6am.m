function [PSNR_DFT_ifdma PSNR_DFT_lfdma y1_ifdma_DFT y1_lfdma_DFT] = DFT_SCFDMA_6am(SP)
...
for sss=1: length(SP.SNR)
    ...
    for jj =1: nloops % loop for columns
        ...
        %%%%%%%%%%%%%%% QPSK Modulation %%%%%%%%%%%%%%%%
        dataSet = [-3+3i -1+3i 1+3i 3+3i -3+1i -1+1i 1+1i 3+1i -3-1i -1-1i 1-1i 3-1i -3-3i -1-3i 1-3i 3-3i];
    ...
    PSNR_DFT_ifdma(sss)=10*log(1/MSE1_ifdma)/log(10);
    figure (3)
    imshow(y1_lfdma)
    MSE1_lfdma=sum(sum((double(im)/255-y1_lfdma).^2))/prod(size(im));
    PSNR_DFT_lfdma(sss)=10*log(1/MSE1_lfdma)/log(10);
