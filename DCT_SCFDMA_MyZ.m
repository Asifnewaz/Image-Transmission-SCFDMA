function [PSNR_DCT_ifdma_Z  PSNR_DCT_lfdma_Z y1_ifdma_DCT_Z y1_lfdma_DCT_Z] = DCT_SCFDMA_MyZ(SP)
...
demodata1_ifdma = zeros(SP.inputBlockSize ,nloops);% this matrix to %store received data vector
demodata1_lfdma = zeros(SP.inputBlockSize ,nloops);
clear jj
for jj =1: nloops % loop for columns
    ...
    %%%%%%%%%%%% SC-FDMA  Modulation %%%%%%%%%%%%%
    inputSymbols_freq = dct(inputSymbols);
    ...
    inputSamples_ifdma = idct(inputSamples_ifdma);
    inputSamples_lfdma = idct(inputSamples_lfdma);
    %%%%%%%%%%%%% Add Cyclic Prefix %%%%%%%%%%%%%
    ...
    Y_ifdma = dct(RxSamples_ifdma, SP.FFTsize);
    Y_lfdma = dct(RxSamples_lfdma, SP.FFTsize);
    ...
    EstSymbols_ifdma = idct(Y_ifdma);
    EstSymbols_lfdma = idct(Y_lfdma);
    %%%%%%%%%%%%%%%% demodulation%%%%%%%%%%%%%%%%
...
PSNR_DCT_ifdma_Z(sss)=10*log(1/MSE1_ifdma)/log(10);
figure (23)
imshow(y1_lfdma)
MSE1_lfdma=sum(sum((double(im)/255-y1_lfdma).^2))/prod(size(im));
PSNR_DCT_lfdma_Z(sss)=10*log(1/MSE1_lfdma)/log(10);
y1_ifdma_DCT_Z(:,:,sss)=y1_ifdma;
y1_lfdma_DCT_Z(:,:,sss)=y1_lfdma;
toc
end