function [PSNR_ofdma y1_ofdma] = OFDMA(SP)
...
for sss=1: length(SP.SNR)
    ...
    for jj =1: nloops % loop for columns
        ...
        inputSamples(1+SP.subband:Q:numSymbols) = inputSymbols;
        inputSamples_ofdma = ifft(inputSamples);
        %%%%%%%%%%%%% Add Cyclic Prefix %%%%%%%%%%%%%
    ...
    %  demodata2 = demodata1(:);
    yy1_ofdma = reshape (demodata1_ofdma,M3,N3);
    %reshaping the matrix to vector
    yy1_ofdma = reshape (demodata1_ofdma,M3,N3);
    %reshaping the matrix to vector
    received_image_ofdma = yy1_ofdma(1:M2*N2);%taking the original data
    ...
    PSNR_ofdma(sss)=10*log(1/MSE1_ofdma)/log(10);
    toc
    y1_ofdma(:,:,sss)=y1_ofdma;
end
