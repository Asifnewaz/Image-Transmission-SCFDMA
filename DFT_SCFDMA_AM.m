...
for jj =1: nloops % loop for columns
    ...
    tmp = b1;
    tmp_coded = convenc(tmp,trel);% Encode the message.
    tmp_coded = matintrlv(tmp_coded.',SP.inputBlockSize/4,8); % Matrix Interleaving
    tmp_coded = tmp_coded.';
    modSymbols = mod_(tmp_coded,'QPSK');
    %%%%%%%%%%%% SC-FDMA  Modulation %%%%%%%%%%%%%
    inputSymbols_freq = fft(modSymbols);
    ...
    elseif SP.equalizerType == 'MMSE'
        C = conj(H_eff)./(conj(H_eff).*H_eff +10^(-SP.SNR/10));
        Y_ifdma = Y_ifdma.*C;
    end
    ...
    %%%%%%%%%%%%%%%% demodulation%%%%%%%%%%%%%%%%
    Est_Symbols_lfdma = sign(real(EstSymbols_lfdma));
    Est_Symbols_lfdma = (Est_Symbols_lfdma+1)/2;
    s1_est_lfdma = matdeintrlv(Est_Symbols_lfdma,SP.inputBlockSize/8,8);
    decoded_lfdma = vitdec(s1_est_lfdma',trel,6,'cont','hard');% Decode
    Est_Symbols_ifdma = sign(real(EstSymbols_ifdma));
    Est_Symbols_ifdma = (Est_Symbols_ifdma+1)/2;
    s1_est_ifdma = matdeintrlv(Est_Symbols_ifdma,SP.inputBlockSize/8,8);
    decoded_ifdma = vitdec(s1_est_ifdma',trel,6,'cont', 'hard');% Decode
    demodata1_ifdma(:,jj) =  decoded_ifdma(:);   % the output of scfdma columns%storing of received image data
    demodata1_lfdma(:,jj)  = decoded_lfdma(:);
...
PSNR_DFT_ifdma=10*log(1/MSE1_ifdma)/log(10);
figure (3)
imshow(y1_lfdma)
MSE1_lfdma=sum(sum((double(im)/255-y1_lfdma).^2))/prod(size(im));
PSNR_DFT_lfdma=10*log(1/MSE1_lfdma)/log(10);
...
%end
%SNR=SP.SNR;
%plot(SNR,PSNR_ifdma_DFT,'ro-',SNR,PSNR_lfdma_DFT,'bo-')
%papr = 10*log10(max(abs(Estsymbols_ifdma).^2)/mean(abs(Estsymbols_ifdma).^2));
%[N X]=hist(papr,1000);
%semilogy (X,1-cumsum(N)),'b-';
%axis([0 20 1e-4 1])
