function [PSNR_DFT_ifdma_Z PSNR_DFT_lfdma_Z y1_ifdma_DFT_Z y1_lfdma_DFT_Z] = DFT_SCFDMA_ZERO(SP)
...
PSNR_DFT_ifdma_Z(sss)=10*log(1/MSE1_ifdma)/log(10);
figure (123)
imshow(y1_lfdma)
MSE1_lfdma=sum(sum((double(im)/255-y1_lfdma).^2))/prod(size(im));
PSNR_DFT_lfdma_Z(sss)=10*log(1/MSE1_lfdma)/log(10);
y1_ifdma_DFT_Z(:,:,sss)=y1_ifdma;
y1_lfdma_DFT_Z(:,:,sss)=y1_lfdma;
